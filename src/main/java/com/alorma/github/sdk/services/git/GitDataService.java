...
public interface GitDataService {
  ...
  @GET("/repos/{owner}/{repo}/git/refs")
  void repoReferences(@Path("owner") String owner, @Path("repo") String name, @Query("page") int page,
  ...
  Observable<GitCommit> repoCommit(@Path("owner") String owner, @Path("repo") String name, @Path("sha") String sha);

  @GET("/repos/{owner}/{repo}/git/trees/{sha}")
  Observable<GitTree> repoTree(@Path("owner") String owner, @Path("repo") String name, @Path("sha") String sha);

  @GET("/repos/{owner}/{repo}/git/trees/{sha}?recursive=1")
  Observable<GitTree> repoTreeRecursive(@Path("owner") String owner, @Path("repo") String name, @Path("sha") String sha);

  @GET("/repos/{owner}/{repo}/git/blobs/{sha}")
  Observable<GitBlob> repoBlob(@Path("owner") String owner, @Path("repo") String name, @Path("sha") String sha);
}
